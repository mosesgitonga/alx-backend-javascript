...
crud.prototype.init = function (database) {
    this.database = database;
}

crud.prototype.crud = function(req, res) {
    var method = req.method;
    var database = crud.database;
    var collection = req.params.collection;
    if (mapping[method] != undefined && (!mappingCollection || mappingCollection[collection] === true)) {
    	mapping[method](database, collection, {body: req.body, query: req.query, params: req.params}, function(err, results) {
    	    if (err) {
...
function put(database, collection, query, callback) {
    var id = null;
    var Model = database.collection(collection);
...
function del(database, collection, query, callback) {
    var id = null;
    var Model = database.collection(collection);
...
function post(database, collection, query, callback) {
    var data = query.body;
    var Model = database.collection(collection);
...
function get(database, collection, query, callback) {
    var errors = [];
    var results = [];
...
var crud = module.exports = exports = new crud;